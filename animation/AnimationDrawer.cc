...
#include "mojgame/animation/AnimationPlayer.h"
...
void AnimationDrawer::Draw(const Animation &anim, float time,
  ...
  if (anim_frame->resource_id < 0) {
    mojgame::LOGGER().Error("Failed to get valid resource id (time: %f)", time);
    return;
  }
...
void AnimationDrawer::Draw(const AnimationPlayer &player,
                           const AnimationResource &resource,
                           const glm::vec2 &translation, float rotation,
                           const glm::vec2 &scaling) {
  if (nullptr != player.anim()) {
    Draw(*(player.anim()), player.anim_time(), resource, translation, rotation,
         scaling);
  };
}
